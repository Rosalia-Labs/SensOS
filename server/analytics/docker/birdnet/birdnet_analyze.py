...
"""
BirdNET analysis service for analytics server.
Processes cataloged audio files from all locations, computing BirdNET scores,
...
logger = logging.getLogger("birdnet-analyzer")

# DB connection
DB_PARAMS: str = (
    f"dbname={os.environ.get('POSTGRES_DB', 'sensos')} "
    f"user={os.environ.get('POSTGRES_USER', 'sensos')} "
    f"password={os.environ.get('POSTGRES_PASSWORD', 'sensos')} "
    f"host={os.environ.get('DB_HOST', 'sensos-analytics-database')} "
...
def get_next_file(cur: psycopg.Cursor) -> Optional[Tuple[int, int, str]]:
    """
    Get next unprocessed file from any location.
    Returns (file_id, location_id, file_path).
    """
    cur.execute(
        """
        SELECT af.id, af.location_id, af.file_path
        FROM sensos.audio_files af
        WHERE af.deleted = FALSE
          AND af.frames IS NOT NULL
...
def fetch_metadata(
    cur: psycopg.Cursor, file_id: int, location_id: int
) -> Optional[Tuple[Path, float, float, Dict[str, Any]]]:
    """
    Fetch file metadata and location coordinates.
    Returns (abs_path, latitude, longitude, audio_metadata).
    """
    cur.execute(
        """
        SELECT af.file_path, l.latitude, l.longitude, l.location_name
        FROM sensos.audio_files af
    ...
    file_path, lat, lon, location_name = row
    
    # Construct absolute path: /audio_recordings/<location>/<file_path>
    # file_path is relative to location root (e.g., "cataloged/2025/04/17/file.flac")
    abs_path = ROOT / location_name / file_path

    try:
        info = sf.info(abs_path)
        return abs_path, lat, lon, {
        ...
        logger.warning(f"Failed to read metadata for {abs_path}: {e}")
        return None


def get_file_and_metadata(
    cur: psycopg.Cursor,
) -> Optional[Tuple[int, int, str, Path, float, float, Dict[str, Any]]]:
    """
    Get next file to process with all necessary metadata.
    Returns (file_id, location_id, file_path, abs_path, lat, lon, audio_meta).
    """
    file_entry = get_next_file(cur)
    if not file_entry:
        return None

    file_id, location_id, file_path = file_entry
    result = fetch_metadata(cur, file_id, location_id)
    if result is None:
        return None

    abs_path, lat, lon, meta = result
    return file_id, location_id, file_path, abs_path, lat, lon, meta


def process_file(
    cur: psycopg.Cursor, 
    file_info: Tuple[int, int, str, Path, float, float, Dict[str, Any]]
) -> None:
    """Process a single audio file, extracting segments and features."""
    file_id, location_id, file_path, abs_path, lat, lon, meta = file_info
    logger.info(
        f"Processing {file_path} ({meta['channels']} ch, {meta['frames']/meta['sample_rate']:.1f} s) "
        f"at ({lat:.4f}, {lon:.4f})"
    )
    with sf.SoundFile(abs_path.as_posix(), "r") as f:
        count = analyze_segments(f, cur, file_id, meta["channels"], lat, lon)
        cur.execute(
...
def analyze_segments(
    ...
    """Analyze all 3-second segments in the audio file."""
    segment_count = 0
    for start in range(0, int(f.frames) - SEGMENT_SIZE + 1, STEP_SIZE):
        ...
        for ch in range(channels):
            ...
            analyze_and_store_features(cur, segment_id, raw_audio, latitude, longitude)
            segment_count += 1
...
def insert_segment(
    ...
    """Insert a new audio segment record and return its ID."""
    cur.execute(
...
def analyze_and_store_features(
    ...
    """
    Compute and store all features for a single segment:
    - Sound statistics (peak, RMS, SNR)
    - Full and bioacoustic spectrum
    - BirdNET embeddings and scores
    - Score statistics (Hill number, Simpson index)
    """
...
def is_valid_metadata(
    ...
    """Verify that file on disk matches database metadata."""
    file_path = None
    try:
        _, _, file_path, abs_path, _, _, meta = file_info
        info = sf.info(abs_path)
        return (
