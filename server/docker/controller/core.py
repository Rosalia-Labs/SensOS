...
def generate_api_controller_configs(
    ...
    cur.execute(
        """
        SELECT name, ip_range, wg_public_key, wg_port
        FROM sensos.networks;
    """
    )
    networks = cur.fetchall()

    for name, ip_range_cidr, server_pub_key, wg_port in networks:
        ...
        # 3) Build the single [Peer] block for the server
        ...
        # 4) Write out (or overwrite) the .conf
        iface.save_config(overwrite=True)

    # 5) Optionally restart the API proxy container
